...
If you’ve been following along, you’re already familiar with the structure of our Tour de France dataset and how to load it into R. If not, check out [the previous post](https://datasharpacademy.com/tour-de-france-meets-data-science-a-beginners-case-study/) for a step-by-step walkthrough.
...
We can now clearly see that the space between the first and last name is a proper space, while the "space" between the name and the country code is something else! Since the odds of findings more non-ASCII characters elsewhere in the table are high, the simplest solution for us to move forward with our analysis is to batch-replace all the non-ASCII characters by a similar ASCII characters (_e.g._ é by e, ù by u, weird space by regular space, etc.)
...
### 3.1 Q1: Which City Has Been Visited Most Often?
...
**Tip**: Pay attention to the spaces around " to ". If you forget them, you’ll get city names like "Paris " or " Lyon", which will trip you up later. "Paris " (space after) and " Paris" (space before) are different, even for the smartest computer.
...
There must be a perfectly logical explanation to this. Let's look for patterns in our data:
...
Turns out, all the blanks are arrival cities. Let’s examine them:
...
If we look at the `Course` column, we can see that only one city name is indicated, which supports the results we got.

But if we look at the `Type` column, here we see a potential reason emerging! The "problematic" stages are time trials, which start and end from the same city. Let's verify that:

```{r echo=TRUE, results='show'}
table(stages[city_names[, 2] == '', 'Type'])
```

Most of them are time trials, indeed, but not all. That brings us to a challenge for you:
...
Bordeaux appears to be the top city, followed by Pau and Paris.

> **But… 🚨 is that really true?**

Look closely and you’ll notice that Paris appears in different forms, like "Paris" and "Paris (Champs-Élysées)". These inconsistencies can break your counts.
...
```
Looks like Bordeaux is consistent. Pau also shows up a few extra times. But are they real?

```{r echo=TRUE, results='show'}
table(city_names[stringr::str_detect(city_names, "Pau")])
```

Nope, they were false positive! So Bordeaux is still the most visited city at this point of the analysis. Even if we strongly feel that Paris will win.
...
### 3.2 Q2: What Are the Smallest and Largest Time Gaps Between the Winner and the Runner-Up of the Tour?

This question takes us into the `Time` column of the `finishers` table. The challenge? The data aren’t clean or consistent.

- For the winner, the Time column contains their total time..
...
> 💡 In many analyses, you’d want to convert anomalies back to absolute values. But in our case, anomalies are exactly what we need! We want to find the smallest and largest time differences for second-place finishers.
...
> **🚴 Tour trivia** Between 1905 and 1912, the Tour was ranked by a point-based system instead of time, due to rampant cheating in the early editions. Curious about the juicy details? Check out the Wikipedia of [the 1905 edition of the Tour de France](https://en.wikipedia.org/wiki/1905_Tour_de_France).
...
We’re now working with 101 editions of the Tour.
...
The `Time` anomalies are stored as complex strings that we need to convert to numbers. Unfortunately, this way of reporting time is non-standard. No built-in R function handles these directly, so we’ll write our own!
...
1. **time_string is our input**: This is one value from the Time column of our dataset. It could look like + 1h 2' 14" or just + 42" depending on the race.
2. **We clean up the string with stri_trans_general()**: This dataset is full of non-ASCII characters and this column is no exception. My first attempts crashed because of the presence of curly quotes instead of regular quotes. While we could have fixed that before using the function, including it in the function body adds an extra security if we ever want to reuse this function in another context.
3. **We remove the “+” sign with substr()**: All our time strings begin with a +, but we don’t need it. Using substr(), we grab everything after the first character. I used `substr()` here as a chance to show you a new tool, though `str_replace()` would also work.
4. **We remove all the spaces**: To make the parsing easier, we remove all whitespace, replacing them with an empty character. Note: `str_replace()` only removes the first space. We want to remove all of them, so we use `str_replace_all()`.
5. **We initialise all time components to zero**: Not all time strings include hours or minutes, so we define `hours`, `minutes`, and `seconds` equal to 0.
6. **We extract hours if they exist**: We check if the string contains "h". If yes, we split it. "5h49'12\"" → ["5", "49'12\""] We save the “5” as the number of hours and update `clean_str` with the second element to work on the remaining part.
7. **We extract minutes and seconds**: We apply the same idea, searching for "'" and "\"" to split the string.
...
>**💬 A Quick Reality Check** I’m showing you the final, polished version here, but it didn’t start that way. It took me almost an hour to get this function working properly. I started with a simple version that worked for one or two clean examples, then slowly added more logic to handle edge cases: "+ " v "+", the absence of minutes or seconds in some elements, the presence of non-ASCII characters (always them!). **That’s how real coding works.** You don’t have to write the perfect function in one go. Start with the ideal case, then adapt as your data shows you its quirks.
...
- `which.min(x)` gives you the position (or row index) of that smallest value.
...
The legendary [Tour de France 1989](https://en.wikipedia.org/wiki/1989_Tour_de_France), where American Greg LeMond snatched victory from Frenchman Laurent Fignon in the final Time Trial stage for 8 tiny seconds. 💛🚴‍♂️

> ✨ This is a great reminder that the beauty of data analysis often lies not in the code itself, but in the stories it helps uncover. 😊



### 3.3 Q3: Who has finished last the most?
...
The last Rider is identified as the rider with the largest rank **for each edition**.

When you hear **for each edition** or similar criteria, you should think of group filtering.

We need a function that does a _specific task_ for _each element of the group_.
...
- A function to apply to each subgroup (here, `tail()` to get the last element),
...
> 🏅 Belgian rider Wim Vansevenant finished last in the Tour de France three times, making him the most frequent lanterne rouge in history. He summerised the value of this performance best : "Lanterne rouge is not a position you go for, it comes for you." ([Source Wikipedia](https://en.wikipedia.org/wiki/Wim_Vansevenant)).
...
2. **When data looks strange, stop and investigate.** A city with no name? A time gap that looks suspicious? Dig deeper. There’s often a logical explanation, but only if you’re curious enough to look.
3. **Use built-in tools to simplify your life.** Functions like tapply(), which.min(), or even str_detect() are powerful allies. Learn what’s available so you don’t have to reinvent the wheel.
3. **Write functions when tasks get repetitive or fiddly.** If you need to repeat the same logic over and over, that’s your cue to write a function. It’s an investment in clarity and reusability.
